<?php
/**
 * Endpoint directo para reservas del landing page
 * Diseñado para funcionar sin configuración del servidor
 */
...
try {
    // Log de la petición
    $logMessage = "[" . date('Y-m-d H:i:s') . "] RESERVA DIRECTA - Petición recibida\n";
    $logMessage .= "Datos POST: " . print_r($_POST, true) . "\n";
    $logMessage .= "Archivos: " . print_r($_FILES, true) . "\n";
    $logMessage .= "Headers: " . print_r(getallheaders(), true) . "\n";
    $logMessage .= "---\n";
    
    // Escribir log a un archivo
    file_put_contents(__DIR__ . '/reserva_log.txt', $logMessage, FILE_APPEND | LOCK_EX);
    ...
    if ($stmt->execute()) {
        ...
        // Log de éxito
        $successLog = "[" . date('Y-m-d H:i:s') . "] RESERVA CREADA EXITOSAMENTE - ID: {$newId}\n";
        file_put_contents(__DIR__ . '/reserva_log.txt', $successLog, FILE_APPEND | LOCK_EX);
        ...
        $errorLog = "[" . date('Y-m-d H:i:s') . "] ERROR: No se pudo ejecutar la consulta SQL\n";
        file_put_contents(__DIR__ . '/reserva_log.txt', $errorLog, FILE_APPEND | LOCK_EX);
...
function uploadFile($file, $tipo) {
    try {
        ...
        $errorLog = "[" . date('Y-m-d H:i:s') . "] ERROR EN UPLOAD: " . $e->getMessage() . "\n";
        file_put_contents(__DIR__ . '/reserva_log.txt', $errorLog, FILE_APPEND | LOCK_EX);
        return '';
    }
}
